...
export const bindForDirective = (mappedLoopElement, elementContext) => {
    ...
    const arrayReferringObject = Object.entries(
        $VindEngine.reactiveVariables
    ).find(([, value]) => {
    ...
    const mountNewLoopElement = (arrayElement) => {
        ...
        if (loopDescriptors.indexName) {
            const element = evaluatedTargetArray.$VindReactiveArray.find(({ element }) => {
            ...
            loopChildElement.attributes.bindingAttributes.forEach(
                ({ name: bindingAttribute, value: attributeValue }) => {
                    ...
                    if (bindingAttribute === ":style") {
                        bindStyleDirective(loopChildElement.element, componentContext);
    ...
    const effect = () => {
        ...
        evaluatedTargetArray.forEach((arrayElement, elementIndex) => {
