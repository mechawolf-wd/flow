...
export const bindForDirective = (mappedLoopElement, elementContext) => {
    ...
    const loopAttributeContent = mappedLoopElement.element.getAttribute(
        "data-vind-origin-of-for"
    );
    ...
    const [arrayReference] = Object.entries($VindEngine.reactiveVariables).find(
        ([key, value]) => value.variableProxy === evaluatedTargetArray
    );
    ...
    const mountNewLoopElement = (arrayElement) => {
        ...
        // marking all nested loops
        const nestedLoops = [...currentLoopedElement.querySelectorAll("loop")];

        nestedLoops.forEach((nestedLoop) => {
            nestedLoop.setAttribute("data-vind-nested-loop", true);
        });

        const loopedElementChildNodes = [
            ...currentLoopedElement.querySelectorAll("*"),
        ];
        ...
        mappedInnerLoopElements.forEach((loopChildElement) => {
            if (Object.keys(loopChildElement) === 0) return;
        ...
        const loopedElement = currentLoopedElement.children[0];
    ...
    const effect = () => {
        ...
        evaluatedTargetArray.forEach((arrayElement) => {
            const children = mountNewLoopElement(
                arrayElement,
                elementContext,
                mappedLoopElement.element
            );

            mappedLoopElement.element.append(...children);
        });
    };

    $VindEngine.reactiveArraysDOMElements[arrayReference].push({
        HTMLCollection: mappedLoopElement.element.children,
    ...
    $VindEngine.queueReactiveEffect(
        { effect },
        { predefinedReferences: [arrayReference] }
    );
};
