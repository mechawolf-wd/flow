...
export function compileComponent(componentFunction = () => { }, componentParentNode = {}, parentContext = {}, componentName = '') {
    ...
    mappedInnerNodes.forEach((mappedInnerNode) => {
        ...
        const isComponentModule = $VindEngine.componentModules[element.tagName]

        // * insertTags
        if (isComponentModule) {
            componentNamesUsed.push(element.tagName)
        ...
        mappedInnerNode.attributes.eventAttributes.forEach(({ name: eventAttribute, value }) => {
            ...
            element.addEventListener(translateEventAttribute(eventAttribute), evaluatedCallaback)
        })
        ...
        mappedInnerNode.attributes.bindingAttributes.forEach(({ name: bindingAttribute, value }) => {
            ...
            element.setAttribute(`data-flow-attribute-original-content-${translatedBindingAttribute}`, value)
            ...
            const isNodeNotAChildOfLoopElement = !element.getAttribute('data-flow-looped-element')
                    ...
                    if (modelAttributeValue) {
                        ...
                        if (proxy) {
                            ...
                            if (inputTypesWithCheckedAttribute.includes(inputType)) {
                                $VindEngine.queueReactiveEffect({
                                    effect: () => {
                                        const condition = evaluateJSExpression(componentContext, `${value}.value`)
                            ...
                            console.warn(`Reactive variable "${modelAttributeValue}" is not defined in the "${componentName}" component context.`)
                        }
