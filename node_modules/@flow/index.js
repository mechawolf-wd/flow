...
export const Flow = {
    render() {
        $FlowEngine.storeGenerationFunctions.forEach((storeGenerationFunction) =>
            storeGenerationFunction()
        );

        callWithErrorOverlay(() => {
            const defaultEntryPointComponentFunction = $FlowEngine.componentModules[DEFAULT_ENTRY_POINT_COMPONENT_NAME.toUpperCase()].componentFunction;
...
window.$FlowEngine = (() => {
    ...
    const runReactiveEffects = ({
        ...
        if ($FlowEngine.lastInfluencedEffectReference === reactiveVariableReference)
            return;
