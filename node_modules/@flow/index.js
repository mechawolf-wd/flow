...
export const Flow = {
    render() {
        callWithErrorOverlay(() => {
            const defaultEntryPointComponentFunction = $FlowEngine.components[DEFAULT_ENTRY_POINT_COMPONENT_NAME].componentFunction;
...
window.$FlowEngine = (() => {
    ...
    const defineComponent = (componentModule) => {
        const componentProps = componentModule["props"];
        const componentEmits = componentModule["emits"];
        ...
        $FlowEngine.props[upperCaseComponentName] ??= componentProps || [];
        $FlowEngine.emits[upperCaseComponentName] ??= componentEmits || [];

        components[upperCaseComponentName] = {
            componentFunction,
            meta: { timesUsed: 0 },
        };
    };
