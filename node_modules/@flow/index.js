...
export const Flow = {
    ...
    defineComponents(componentModules) {
        componentModules.forEach((componentModule) => {
            $FlowEngine.defineComponent(componentModule);
        });
    },
    defineStores(stores) {
...
window.$FlowEngine = (() => {
    ...
    const runEffects = ({ reference: reactiveVariableReference, configuration = {} }) => {
        // ! This is called three times anyways.
        if ($FlowEngine.watchCallbacks[reactiveVariableReference].length > 0 && configuration.isComputed) {
            const newComputedValue = $FlowEngine.reactiveVariables[reactiveVariableReference].value

            $FlowEngine.watchCallbacks[reactiveVariableReference].forEach(watchSignal => watchSignal(newComputedValue, newComputedValue))
        }

        effects.forEach(({ references: effectReferences, effect: effectCallback }) => {
    ...
    // Components
    const defineComponent = (componentModule) => {
        const componentProperties = componentModule["props"] || [];
        const componentEvents = componentModule["emits"] || [];

        // ! This is not checking whether the function is an actual "component function".
        ...
        $FlowEngine.propertiesByComponent[upperCaseComponentName] ??= componentProperties;
        $FlowEngine.eventsByComponent[upperCaseComponentName] ??= componentEvents;
