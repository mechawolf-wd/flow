                ...
                return function (...methodArguments) {
                    if (property === 'push' || property === 'unshift' || property === 'pop' || property === 'shift') {
                        requestAnimationFrame(() => {
                            $FlowEngine.runEffects({ reference })
                        })
                    }
...
const queueReferenceExtracting = (computedCallback, variableReference, configuration) => {
    ...
    $FlowEngine.currentSignalReference = variableReference

    computedCallback()

    configuration.dependsOn.push(Array.from($FlowEngine.extractedDependencies))

    $FlowEngine.currentSignalReference = ''
    ...
    const variableProxy = new Proxy({ value, configuration, variableReference }, {
        get(target, property) {
            ...
            if ($FlowEngine.computedDependencyExtractorRunning) {
                ...
                target.configuration.influences.push($FlowEngine.currentSignalReference)
            ...
            if (typeof value === 'function') {
                ...
                $FlowEngine.watchCallbacks[variableReference].forEach(watchSignal => watchSignal(newComputedValue, newComputedValue))
            }
        ...
        set(target, property, newValue) {
            ...
            // ---- Checking if the updates are necessary ----
            if (isPrimitiveValue && newValue === oldValue) {
                return true
            }
            ...
            $FlowEngine.runEffects({ reference: variableReference, configuration })
