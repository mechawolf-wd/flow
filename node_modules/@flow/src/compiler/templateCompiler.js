...
export async function compileComponent(componentFunction = () => { }, componentParentNode = {}, parentContext = {}, componentName = '') {
    ...
    const componentContextMethods = {
        ...
        stores: $FlowEngine.stores,
        useRouter: () => {
            const routerStore = $FlowEngine.stores.routerStore
                ...
                if ($FlowEngine.extractedDependencies.size > 0) {
                    ...
                    if (extractedDependencies.length > 0) {
                        ...
                        extractedDependencies.forEach((extractedDependency) => {
                            $FlowEngine.watchCallbacks[extractedDependency].push(callback)
                        })
                    }
    ...
    componentContext.$path = $FlowEngine.stores.routerStore.path
    ...
    if (!$FlowEngine.mountedStyleByComponent[upperCaseComponentName]) {
        mountStyling(componentStyle, upperCaseComponentName)

        $FlowEngine.mountedStyleByComponent[upperCaseComponentName] = true
    }
...
export function templateCompiler(componentFunction, lookUpNode, parentContext, parentComponentName, defaultEntryPointId) {
    ...
    if (defaultEntryPointId) {
        ...
        componentNodes = Array.from(foundNodes)
    } else {
    ...
    componentNodes.forEach(async (componentNode) => {
        const { componentNodeReference, componentNamesUsed, nextParentContext } = await compileComponent(componentFunction, componentNode, parentContext, parentComponentName)
