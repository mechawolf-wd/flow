...
export function compileComponent(componentFunction = () => { }, componentParentNode = {}, parentContext = {}, componentName = '') {
    ...
    const componentContextMethods = {
        ...
        watch: (source, callback, watchConfiguration) => {
            if (!source.configuration?.isReactiveVariable && typeof source !== 'function') return
            ...
            if (typeof source === 'function') {
                ...
                if ($FlowEngine.extractedDependencies.size > 0) {
                    const extractedDependencies = Array.from($FlowEngine.extractedDependencies);
                    ...
                    if (extractedDependencies.length > 0) {
                        ...
                        $FlowEngine.watchCallbacks[firstReferencedReference].push(callback)
                    }
    ...
    componentContext.$emit = (eventName) => () => componentContextMethods.emit(eventName)
...
export function templateCompiler(componentFunction, lookUpNode, defaultEntryPointId, previousParentContext, parentComponentName) {
    let componentNodes = []

    if (defaultEntryPointId) {
        componentNodes = [...lookUpNode.querySelectorAll(defaultEntryPointId)]
    } else {
        componentNodes = [...lookUpNode.querySelectorAll(`${parentComponentName}:not([data-flow-compiled-component])`)]
    }

    componentNodes.forEach((componentNode) => {
        const { componentNodeReference, componentNamesUsed, nextParentContext } = compileComponent(componentFunction, componentNode, previousParentContext, parentComponentName)
        ...
        componentNamesUsed.forEach(usedComponentName => {
            if (!$FlowEngine.componentModules[usedComponentName]) return

            const componentModule = $FlowEngine.componentModules[usedComponentName]

            const nextComponentName = $FlowEngine.componentModules[usedComponentName].componentName

            templateCompiler(componentModule.componentFunction, componentNode, undefined, nextParentContext, nextComponentName)
        })
    })
}