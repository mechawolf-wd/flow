...
export function compileComponent(componentFunction = () => { }, componentParentNode = {}, parentContext = {}) {
    ...
    const mappedInnerDrawerTags = drawerNodes.map(drawerNode => {
        ...
        if (targetInsertTag) {
            drawerNode.prepend(...targetInsertTag.insertNode.childNodes)
        }
    ...
    mappedInnerNodes.forEach((mappedInnerNode) => {
        ...
        mappedInnerNode.attributes.bindingAttributes.forEach(({ name: bindingAttribute, value }) => {
            ...
            if (isNodeNotAChildOfLoopElement) {
                ...
                const isNodeATextInput =
                    bindingAttribute === ':value' &&
                    mappedInnerNode.element.tagName === 'INPUT' &&
                    (mappedInnerNode.element.getAttribute('type') === 'text' || !mappedInnerNode.element.hasAttribute('type'))

                if (isNodeATextInput) {
                    $FlowEngine.queueReactiveEffect({
                        effect: () => {
                            mappedInnerNode.element.value = evaluateJSExpression(componentContext, value)
                        }
                    })
                ...
                const isNodeACheckboxInput =
                    bindingAttribute === ':checked' &&
                    mappedInnerNode.element.tagName === 'INPUT' &&
                    mappedInnerNode.element.getAttribute('type') === 'checkbox'
