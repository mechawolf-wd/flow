...
export function compileTemplate(componentFunction, componentName, parentNode) {
    ...
    const componentContextMethods = {
        ...
        computed: (value) => {
            return reactiveVariable(value, { isStored: false, isRef: false, isComputed: true, influences: [], dependsOn: [] })
        },
        useStore: (storeName) => {
    ...
    const componentsUsed = determineUsedComponents(componentContext.template)

    return { componentNode: componentNodeReference, componentsUsed }
}

export function templateCompiler(componentName, componentFunction, entryPoint = document.body) {
    if (!componentFunction) return

    // Selecting all component nodes that are defined inside the entryPoint, eg. <Counter></Counter>.
    const componentNodes = [...entryPoint?.querySelectorAll(`${componentName}:not([compiled-flow-component])`)];

    componentNodes.forEach((componentEntryPoint) => {
        const { componentNode, componentsUsed } = compileTemplate(componentFunction, componentName, componentEntryPoint)
        ...
        componentsUsed.forEach(componentName => {
            if (!$Flow.components[componentName]) return

            $Flow.components[componentName].metaData.timesUsed += 1

            templateCompiler(componentName, $Flow.components[componentName].componentFunction, componentEntryPoint)
        });
    })
}