...
export function generateComponent(componentDefinition, componentName, parentNode) {
    ...
    const componentInput = {
        dispatchEvent: (eventName, payload, eventInitDict = { bubbles: true }) => {
        ...
        getStore: (name) => {
            const store = $Flow.getStore(name)
    ...
    const elementAndAttributes = allInnerElements.map((element) => {
    ...
    loopBlocks.forEach(block => {
        ...
        loopBlockElements.forEach((loopBlockElement) => {
            loopBlockElement.setAttribute('flow-looped-element', true)
        })
    })
    ...
    paintBlocks.forEach(block => {
        block.setAttribute('flow-paint-expression', block.textContent)
    })

    paintBlocks.forEach(block => {
        const effect = () => {
            const expression = block.getAttribute('flow-paint-expression')
    ...
    // ==== MOUNTING STEP ====
    elementAndAttributes.forEach((reference) => {
        ...
        reference.attributes.bindingAttributes.forEach(({ name, value }) => {
                ...
                targetIterable.forEach((item) => {
                    ...
                    customLoopAttributes.forEach(loopNode => {
                        loopNode.attributes.eventAttributes.forEach(({ name, value }) => {
                            loopNode.element.addEventListener(translateEventAttribute(name), evaluateExpression({ ...component, [descriptors.itemName]: item }, value))
                        })
                        ...
                        loopNode.attributes.bindingAttributes.forEach(({ name, value }) => {
                            if (name === ':if') {
                                $Flow.queueEffect({
                                    references: uniqueReferences,
                                    effect: () => {
                                        const display = Boolean(evaluateExpression({ ...component, [descriptors.itemName]: item }, value))
                            ...
                            if (name === ':value' && loopNode.element.tagName === 'INPUT') {
                                $Flow.queueEffect({
                                    references: uniqueReferences,
                                    effect: () => {
                                        loopNode.element.value = evaluateExpression({ ...component, [descriptors.itemName]: item }, value)
                                    }
                                })
                            ...
                            $Flow.queueEffect({
                                references: uniqueReferences,
                                effect: () => {
                                    loopNode.element.setAttribute(translateBindingAttribute(name), evaluateExpression({ ...component, [descriptors.itemName]: item }, value))
                                }
                            })
                        })
                    })
                    ...
                    paintForBlocks.forEach(block => {
                        const effect = () => {
                            ...
                            block.textContent = evaluateExpression({ ...component, [descriptors.itemName]: item }, expression)
                        }
            ...
            const isNotAChildOfLoop = !reference.element.hasAttribute('flow-looped-element')
            ...
            if (isNotAChildOfLoop) {
                ...
                if (name === ':value' && reference.element.tagName === 'INPUT') {
                    $Flow.queueEffect({
                        references: uniqueReferences,
                        effect: () => {
