...
import { extractComputedDependencies } from '../utils/extractComputedDependencies.js'
...
export function compileTemplate(componentDefinition, componentName, parentNode) {
    ...
    const componentContextMethods = {
        ...
        ref: (value) => {
            return reactiveVariable(value, { isStored: false, isRef: true, influences: [] })
        },
        computed: (value) => {
            const computedDependencies = extractComputedDependencies(value)

            return reactiveVariable(value, { isStored: false, isComputed: true, dependsOn: computedDependencies, influences: [] })
        },
        useStore: (storeName) => {
    ...
    // ------------ DETERMINING REFS' INFLUENCES ARRAYS ------------
    Object.entries(componentContext).forEach(([key, value]) => {
        const isReactiveVariable = value?.configuration?.isReactiveVariable

        if (!isReactiveVariable) return

        const isComputed = value?.configuration?.isComputed

        if (isComputed) {
            value.configuration.dependsOn.forEach((dependency) => {
                const isRef = componentContext[dependency]?.configuration?.isRef
                const isComputed = componentContext[dependency]?.configuration?.isComputed

                if (isRef || isComputed) {
                    const proxy = componentContext[dependency]

                    proxy.configuration.influences.push(value.variableReference)
                }
            })
        }
    })
    ...
    mappedInnerElements.forEach((componentInnerNode) => {
        ...
        componentInnerNode.attributes.bindingAttributes.forEach(({ name: attribute, value }) => {
            if (attribute === ':for') {
                const effect = () => {
                    ...
                    evaluatedTargetArray.forEach((arrayElement) => {
                        ...
                        paintForBlocks.forEach(paintBlock => {
                            ...
                            const dependencies = paintBlock['data-flow-paint-expression-dependencies'] ??= getExpressionDependencies(paintBlock.textContent)
                ...
                $Flow.queueEffect({
                    // TODO: This effect's reference is the looped array's reference
            ...
            Object.entries(componentContext).forEach(([key, value]) => {
                if (value.configuration?.isReactiveVariable && dependencies?.includes(key)) {
                    attributeDependencies.push(value.variableReference)
                }
            })
            ...
            if (isNodeNotAChildOfLoopElement) {
                if (attribute === ':if') {
                    $Flow.queueEffect({
                        references: attributeDependencies,
                        effect: () => {
