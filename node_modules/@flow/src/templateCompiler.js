...
const camelize = string => string.replace(/-./g, character => character[1].toUpperCase())
...
export function compileComponent(componentFunction = () => { }, componentParentNode = {}, parentContext = {}) {
    ...
    let componentProperties = $FlowEngine.props[componentParentNode.tagName] || [] //  - in case the parent node's tagName is 'DIV'.
    ...
    componentProperties.forEach((propertyName) => {
        ...
        const camelCasePropertyName = camelize(propertyName)
    ...
    loopNodes.forEach(block => {
        ...
        loopBlockElements.forEach((loopBlockElement) => {
            loopBlockElement['data-flow-looped-element'] = true
        })
    })

    // Determining which components are used in the component's template and converting their names to uppercase.
    const componentNamesUsed = determineUsedComponents(componentTemplate).map(componentName => componentName.toUpperCase())
    ...
    mappedInnerElements.forEach((mappedInnerNode) => {
        ...
        mappedInnerNode.attributes.bindingAttributes.forEach(({ name: bindingAttribute, value }) => {
            ...
            const isNodeNotAChildOfLoopElement = !mappedInnerNode.element['data-flow-looped-element']
            ...
            if (isNodeNotAChildOfLoopElement) {
                ...
                $FlowEngine.queueEffect({
                    references: attributeReferences,
                    effect: () => {
                        mappedInnerNode.element.setAttribute(translatedBindingAttribute, evaluateJSExpression(componentContext, value))
                    }
                })
            }
        })
...
export function templateCompiler(componentFunction, lookUpNode, defaultEntryPointId, previousParentContext) {
    ...
    componentNodes.forEach((componentParentNode) => {
        const { componentNodeReference, componentNamesUsed, parentContext } = compileComponent(componentFunction, componentParentNode, previousParentContext)
