...
export function compileTemplate(componentDefinition, componentName, parentNode) {
    ...
    const componentInput = {
        ...
        ref: (value) => {
            return Array.isArray(value) ? createReactiveArray(value, newComponentID) : reactiveVariable(value, newComponentID, { isStored: false })
        },
        computed: (value) => {
    ...
    allElementsAndTheirAttributes.forEach((reference) => {
        ...
        reference.attributes.bindingAttributes.forEach(({ name, value }) => {
            ...
            const isElementNotAChildOfLoop = !reference.element['data-flow-looped']

            if (isElementNotAChildOfLoop) {
                if (name === ':if') {
                    $Flow.queueEffect({
                        references: uniqueReferences,
                        effect: () => {
                ...
                const isAnCheckboxInput = name === ':checked' && reference.element.tagName === 'INPUT' && reference.element.getAttribute('type') === 'checkbox';
...
export function templateCompiler(componentDefinition, entryPoint = document.body) {
    ...
    nodes.forEach((node) => {
        const componentNode = compileTemplate(component, componentName, node)
