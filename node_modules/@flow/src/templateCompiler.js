...
export function compileTemplate(componentFunction, componentName, parentNode) {
    ...
    const componentContext = componentFunction(parentNode.attributes, componentContextMethods)

    // ------------ LIFECYCLE HOOK onBeforeMount ------------
    lifecycleCallbacks.onBeforeMount?.()

    // ------------ PARSING HTML ------------
    componentNodeReference.innerHTML = replaceInterpolationMarkers(componentContext.template)
    ...
    // ------------ MOUNTING PROPS ------------
    // const wrappingNode = parentNode.parentNode

    // const componentInstances = wrappingNode.querySelectorAll(componentName)

    // ------------ MOUNTING STEP ------------
    mappedInnerElements.forEach((componentInnerNode) => {
        componentInnerNode.attributes.eventAttributes.forEach(({ name: key, value }) => {
            // TODO: If there are multiple event attributes with the same name, they should be added to the same event listener.
            // as for now there are two listeners added for the same event.
            // if we restrict adding the same event listener twice, it won't help. eg. the first <Counter /> won't work.
    ...
    mountStyling(componentContext.style, componentNodeReference)

    const componentNamesUsed = determineUsedComponents(componentContext.template)

    return { componentNodeReference, componentNamesUsed }
}

export function templateCompiler(componentName, componentFunction, lookUpNode, defaultEntryPointId) {
    ...
    if (lookUpNode === document.body && defaultEntryPointId) {
        ...
        componentNodes = [...lookUpNode.querySelectorAll(`${componentName}:not([compiled-flow-component])`)];
    }

    componentNodes.forEach((componentEntryPoint) => {
        const { componentNodeReference, componentNamesUsed } = compileTemplate(componentFunction, componentName, componentEntryPoint)

        componentEntryPoint.prepend(componentNodeReference)

        if (componentNamesUsed.includes(componentName)) {
            throw new Error(`Component "${componentName}" tries to render itself.`)
        }

        componentEntryPoint.setAttribute('compiled-flow-component', true);

        componentNamesUsed.forEach(componentName => {
            if (!$FlowEngine.components[componentName]) return

            const componentDefinition = $FlowEngine.components[componentName]

            componentDefinition.meta.timesUsed += 1

            templateCompiler(componentName, componentDefinition.componentFunction, componentEntryPoint)
        });
    })
}