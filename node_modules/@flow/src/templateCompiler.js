...
export function compileTemplate(componentFunction, componentParentNode, parentContext) {
    ...
    // ------------ PROPS BINDING ------------
    let componentProps = $FlowEngine.props[componentParentNode.tagName]

    const props = componentParentNode.attributes.props = {}

    componentProps?.forEach((propName) => {
        const prop = props[propName] = reactiveVariable(undefined, structuredClone(RV_REF_CONFIG))

        prop.value = evaluateJSExpression(parentContext, componentParentNode.getAttribute(`data-flow-attribute-expression-origin-${propName}`))
    })

    const componentContext = componentFunction(componentContextMethods, componentParentNode.attributes?.props)
    ...
    mappedInnerElements.forEach((componentInnerNode) => {
        ...
        componentInnerNode.attributes.bindingAttributes.forEach(({ name: attribute, value }) => {
            componentInnerNode.element.setAttribute(`data-flow-attribute-expression-origin-${translateBindingAttribute(attribute)}`, value)
...
export function templateCompiler(componentFunction, lookUpNode, defaultEntryPointId, previousParentContext) {
    ...
    componentNodes.forEach((componentParentNode) => {
        ...
        componentParentNode.setAttribute('compiled-flow-component', true);
