...
export function compileTemplate(componentFunction, componentParentNode, parentContext = {}) {
    ...
    componentProps.forEach((propName) => {
        const prop = props[propName] = reactiveVariable(undefined, structuredClone(RV_PROP_CONFIG))

        const dependencies = getExpressionDependencies(componentParentNode.getAttribute(`data-flow-attribute-original-content-${propName}`))
        ...
        $FlowEngine.queueEffect({
            ...
            effect: () => {
                prop.value = evaluateJSExpression(parentContext, componentParentNode.getAttribute(`data-flow-attribute-original-content-${propName}`))
            }
        })
    })
